...
In order to place an ad, the advertiser must present a valid ticket to the advertiser. We discuss the details on the tickets in sections below. Each ad added to the table is stored for `target-ad-lifetime`. When the `target-ad-lifetime` expires, the ad is removed. Topic table is structured as a set of per-topic, FIFO queues. An advertiser can only have a single ad for a specific topic queue in the topic table; duplicate placements are rejected (although the same node may attempt placing ads for multiple topics at the same registrar).
...
### Tickets
In order to place an ad on a registrar's topic table, the advertiser must present a valid **'ticket'** to the registrar. Tickets are immutable objects issued by the registrars. An advertiser willing to register an ad at a registrar must first obtain a ticket by sending a 'ticket request' message to the registrar. In response to the ticket request, the registrar issues an initial ticket containing a 'waiting time' and sends the ticket to the advertiser in a 'ticket response' message. The advertiser can come back to the registrar (to register an ad) after the waiting time has elapsed and present the ticket in a 'topic registration request' (i.e., REGTOPIC) message. 

Any REGTOPIC messages that are sent before the waiting time (indicated in the ticket) are ignored by the registrars. If the advertiser comes back to the registrar after the waiting time, the advertiser can either place the ad (and notify the advertiser of a successful registration) or issue another ticket with a new waiting time in another ticket response message. An advertiser may be given one or more tickets in a sequence before a successful registration, and this means that the advertiser waits for a 'cumulative waiting time' period that is the sum of multiple waiting times issued in each ticket in the sequence before successfully registering an ad. Assignment of 'waiting times' is the only way the registrars can control the registrations in order to both:
...
In addition to the waiting time, the sequence of tickets issued by a registrar for a specific advertiser also records the total (cumulative) time waited by the advertiser so far; that is, the time elapsed since the advertiser requested its first ticket to place its ad. The inclusion of cumulative waiting time allows the registrars to prioritise tickets that have been waiting the most as we explain later. Because the tickets are immutable and can not be tampered with by the advertisers, when a registrar issues a new ticket (in case a registration is not immediately successful), the registrar simply increments the cumulative waiting time in the previous ticket (presented in a registration request by the advertiser) with the new waiting time to produce the new cumulative waiting time in the new ticket. This means that the registrars are not required to maintain any state for each on-going ticket request given that they can simply verify the authenticity (and integrity) of the ticket in the incoming registration requests. The registrars ensure the authenticity of the tickets they issue to the advertisers through symmetric encryption. 

Ads should remain in the queue for a constant amount of time, the `target-ad-lifetime`. To maintain this guarantee, new registrations are throttled and the advertisers must wait for a certain amount of time before they are admitted. When a node attempts to place an ad, it receives a 'ticket' which tells them how long they must wait before they will be accepted. It is up to the advertiser node to keep the ticket and present it to the registrar when the waiting time has elapsed. Waiting times calculation are based on few considerations, described in [here](#waiting-time-function) 
...
Implementations may choose to include arbitrary other information in the ticket in addition to the cumulative waiting time spent by the advertiser. A practical way to handle tickets is to
...
Tickets cannot be used beyond their lifetime. If an advertiser does not come back after the waiting time, all cumulative waiting time is lost and the advertiser must start over. 
...
However, these naive approaches are not efficient in terms of overhead, search time or uniform distribution of discovered nodes, or do not follow the requirements described in [here](requirements.md).

In order for the advertisers to keep track of on-going ad registrations on different registrars, a ticket table structure is used. An advertiser follows the steps below to distribute its ads for a specific topic: 
...
2.  A REGTOPIC request is initially sent to each of the selected registrar nodes in the previous step. These initial REGTOPIC messages do not contain a TICKET since the advertiser had not been previously issued any tickets by the chosen nodes.
3.  Registrar node replies with a TICKET response. This message includes the TICKET which contains a waiting time and a ticket issue time.
4.  The advertiser replies after the waiting time expires with another REGTOPIC request with the previously received TICKET attached to it.
5.  The registrar replies with a new TICKET response with a new TICKET and a new waiting time (together with a cumulative waiting time) in case the registration is not succesful. 
6.  The registration is successful when the [waiting time calculated](#waiting-time-function) at the registrar is not greater than the cumulative waiting time of the new ticket. In this case the registrar sends a REGCONFIRMATION response to the advertiser. If the registration is not successful, the advertiser continues the process from step 4.
7.  The process is restarted with a new selected node in the same bucket after the advertisement expires in case of either a successful registration or too many unsuccessful attempts. A registrar gives up and stops the registration process with a registrar upon `T` unsuccessful registration attempts (i.e., being issued T tickets without success).

With the above process, an advertiser initially starts K simultaneous registrations in each bucket. The objective is to establish and maintain K active (i.e., unexpired) registrations in each bucket. This objective is achieved by the advertiser setting a timer with a duration of ad-life-time upon each successful registration with a node in a bucket b, and once the timer expires (after ad-life-time passes) the advertiser starts a new registration with a new node in the bucket b. The ticket table is used to store the tickets obtained for each on-going registrations and the expiration time of active registrations. 
...
base_time = `10 * ad-life-time` / ( 1 -  ( (topic table size / topic_table_capacity) ^ 10) )
```
The base time is determined based on the ad-life-time of an ad and the current occupancy of the table. Longer `ad-life-time` yields longer waiting times, while increased capacity of the table yields shorter waiting time. However, the latter is compensated by the modifiers explained below. 
...
```
All the modifiers increase with increasing number of the same items already in the table. Thus it's getting increasingly difficult to register ads for the same IP/ID/topic. For instance, ads for less popular topic will receive lower waiting times than popular ones. Note that the table does not prevent anyone from registering, but rather makes it slower for already popular items. Such a mechanism promotes diversity in the table and protects against Sybil attacks so that an attacker who is in control of a limited pool of IP addresses won't be able to dominate the table with many ads. The low exponent for the `topic_modifier` is motivated by the topics in the network that are likely to follow a zipf distribution. In contast, honest nodes IPs/IDs should follow a uniform distribution. 
